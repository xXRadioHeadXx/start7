...
//    qDebug() << "PortManager::procT4KMDDStatusWord0x32() -- isChangedStatus " << isChangedStatus;
...
//    qDebug() << "PortManager::procT4KMDDStatusWord0x32() -- isSwitchReady " << isSwitchReady;
...
//    qDebug() << "PortManager::procT4KMDDStatusWord0x32() -- isFirstWakeUp " << isFirstWakeUp;
...
//    qDebug() << "PortManager::procT4KMDDStatusWord0x32() -- isWakeUp " << isWakeUp;
...
//    qDebug() << "PortManager::procT4KMDDStatusWord0x32() -- needResetFlags0x24" << needResetFlags0x24;
    if(needResetFlags0x24) {
        auto msMsg = QSharedPointer<ManagerSingleMsg>::create(currentUN,
                                                              DataQueueItem::makeResetFlags0x24);
        reciverBOD->pushBackUniqManagerSingleMsg(msMsg);
    }
//    qDebug() << "PortManager::procT4KMDDStatusWord0x32() -- makedResetFlags0x24" << makedResetFlags0x24;
    ...
    && (swpCurrent.y(y4).dd(ddNum).isWasOpened() == swpPrevious.y(y4).dd(ddNum).isWasOpened()
     || isSwitchReady)) {
        ...
        //    bool iniState = false;
            ...
            && (swpCurrent.y(y4).dd(ddNum).c(1).isFault() != swpPrevious.y(y4).dd(ddNum).c(1).isFault()
             || isSwitchReady)) {
                commentMsg = QObject::tr("Неисправность по ЧЭ1");
                typeMsg = 12;
                currentUN->setPublishedState(12);
            ...
            && (swpCurrent.y(y4).dd(ddNum).c(2).isFault() != swpPrevious.y(y4).dd(ddNum).c(2).isFault()
             || isSwitchReady)) {
                commentMsg = QObject::tr("Неисправность по ЧЭ2");
                typeMsg = 13;
