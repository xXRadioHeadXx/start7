...
ProcessingStateWord0x41SSOIBLIPSD::~ProcessingStateWord0x41SSOIBLIPSD()
{

}

bool ProcessingStateWord0x41SSOIBLIPSD::processing(const StateWord &data, const QSharedPointer<UnitNode> &currentUN) const
{
...
//        qDebug() << "PortManager::procSDBLIPStatusWord0x41(5) <--";
    ...
    const auto& swpCurrent = currentUN->swpSSOISDBLIPType0x41(),
               &swpPrevious = previousUN->swpSSOISDBLIPType0x41();
...
//       1 == swpCurrent.isAlarm() &&
       (1 == swpCurrent.isWasAlarm() || 1 == swpCurrent.isWasOpened())) { // сброс тревоги
