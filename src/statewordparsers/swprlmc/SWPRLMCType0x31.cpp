...
float SWPRLMCType0x31::threshold() const
{
    if(byteWord().isEmpty())
        return -1.0;
    switch (static_cast<uint8_t>(byteWord().at(2)) & static_cast<uint8_t>(0x0F)) {
...
int SWPRLMCType0x31::clockPeriod() const
{
    if(byteWord().isEmpty())
        return -1;
    return (static_cast<uint8_t>(byteWord().at(2)) & static_cast<uint8_t>(0x70)) >> 4;
}
...
int SWPRLMCType0x31::modeProcessing() const
{
    if(byteWord().isEmpty())
        return -1;
    return static_cast<uint8_t>(byteWord().at(3)) & static_cast<uint8_t>(0x03);
}
